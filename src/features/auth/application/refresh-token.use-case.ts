...
export class RefreshToken implements ICommandHandler<RefreshTokenCommand>{
    ...
    async execute(command: RefreshTokenCommand): Promise<Contract<null | LoginOutputModel>> {
        ...
        const newTokens = await device.refreshDevice({ deviceIp: command.deviceIp, userAgent: command.userAgent, device, accessJwtSecret, refreshJwtSecret })
