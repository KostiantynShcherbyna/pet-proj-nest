...
export class PasswordRecovery implements ICommandHandler<PasswordRecoveryCommand> {
  ...
  async execute(command: PasswordRecoveryCommand): Promise<Contract<null | boolean>> {
    ...
    const newRecoveryCodeDocument = await this.RecoveryCodesModel.createPasswordRecovery(command.email, passwordRecoveryCodeSecret, this.tokensService, this.RecoveryCodesModel,)
