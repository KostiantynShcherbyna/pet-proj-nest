...
import { DevicesQueryRepositorySql } from "../repository/sql/devices.query.repository.sql"
...
@Controller("security")
export class DevicesControllerSql {
  constructor(
    private commandBus: CommandBus,
    protected devicesQueryRepositorySql: DevicesQueryRepositorySql,
  ) {
  }
  ...
  async getDevices(
    ...
    return await this.devicesQueryRepositorySql.findDevicesByUserId(deviceSession.userId)
  }
  ...
  async deleteOtherDevices(
    ...
    const result = await this.commandBus.execute(
      new DeleteOtherDevicesCommandSql(
  ...
  async deleteSpecialDevice(
    ...
    const result = await this.commandBus.execute(
      new DeleteSpecialDeviceCommandSql(
        param.deviceId,
        deviceSession.userId
      )
    )
