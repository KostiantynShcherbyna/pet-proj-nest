...
export class AuthService {
  ...
  async registration(registrationBody: BodyRegistrationModel): Promise<Contract<null | boolean>> {
    ...
    if (isSend === false) {
      const deletedUsers = await this.UsersModel.deleteOne({ _id: newUser._id })
      if (deletedUsers.deletedCount === 0) return new Contract(null, ErrorEnums.USER_NOT_DELETE)
  ...
  async confirmationResend(email: string): Promise<Contract<null | boolean>> {
    ...
    await this.usersRepository.saveDocument(user)
    // SENDING EMAIL ↓↓↓
    const isSend = await emailManager.sendConfirmationCode(user)
    if (isSend === false) {
